...
void searchStudent(struct student* base, int size, const char* type, const char* value1, const char* value2) {
    ...
    printf("\nпоиск %s: ", type);
    if (strcmp(type, "поимени") == 0) printf("%s %s\n", value1, value2);
    else printf("%s\n", value1);

    for (int i = 0; i < size; i++) {
        int found = 0;

        if (strcmp(type, "поимени") == 0) {
            found = (strcmp(base[i].famil, value1) == 0 && strcmp(base[i].name, value2) == 0);
        }
        else if(strcmp(type, "пофакультету") == 0) {
    ...
    if (count == 0) printf("студентненайден\n");
}
...
int main() {
    ...
    searchStudent(base, 5, "пофакультету", "ИБ", "");
    searchStudent(base, 5, "пофакультету", "ВТ", "");
